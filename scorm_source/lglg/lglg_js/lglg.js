...
//****element de base
	...
	for (var i = 0 ; i < 4 ; i++){ 
		...
		if (tblExo.btnOutil.liste[i].fonction === "sousTitrage1"){affichST = true;}
	}
	...
	//*** col droite
	if (tblExo.pages.length > 1){	$("#ctnNavigation").load("lglg/lglg_interface/navigQ.html");}
	...
	for (var i = 0 ; i < tblExo.pages[pCourante].questions.length ; i++){
		if (tblExo.pages[pCourante].questions[i].type == "qo"){
			...
			for (var j = 0 ; j < tblReponses[pCourante].reps[i].length ; j++){
				if  ( tblReponses[pCourante].reps[i][j].length < tblExo.scenario.qoAccesC ){ accesCor = false;}
			}
		}
	}
	if (accesCor) {$("#nav_3").addClass("actif");} else {$("#nav_3").removeClass("actif");}
		...
		for (var indP2 = 0 ; indP2 < tblReponses.length ; indP2++){
			for (var indQ2 = 0 ; indQ2 < tblReponses[indP2].reps.length ; indQ2++){
				if (tblExo.pages[indP2].questions[indQ2].type === "qo"){
					if (tblReponses[indP2].reps[indQ2].length > tblExo.scenario.qoAccesC  ) {nbRepSaisies ++ ;}
					nbQ +=1 ;
				}
			}
		}
...
function lireExtraitVid(e){ 
	...
	var extrait;
	if (params[1] === "c") { extrait="extraitCorrection";} else { extrait="extraitQuestion";}
	...
	if (tblExo.interface){
		...
		for (var i = 0; i < elms.length; i++)	{
			...
			if (subdoc){
				...
				if (subElt.length > 0) {subdocTrouve += 1;}
